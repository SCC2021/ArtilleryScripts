...
var statsRegExpr = [ [/.*\/rest\/media\/.*/,"GET","/rest/media/*"],
			...
			[/.*\/rest\/users\/.*\/channels/,"GET","/rest/user/*/channels"],
			[/.*\/rest\/users\/.*/,"GET","/rest/user/*"],
			[/.*\/rest\/users\/.*/,"DELETE","/rest/user/*"],
			[/.*\/rest\/users\/.*\/subscribe\/.*/,"POST","/rest/user/*/subscribe/*"],
			[/.*\/rest\/users\/auth/,"POST","/rest/user/auth"],
			[/.*\/rest\/users/,"POST","/rest/user"],
			[/.*\/rest\/channels\/.*\/add\/.*/,"POST","/rest/channel/*/add/*"],
			[/.*\/rest\/channels/,"POST","/rest/channel"],
			[/.*\/rest\/channels\/.*\/messages.*/,"GET","/rest/channel/*/messages"],
			[/.*\/rest\/channels\/.*/,"GET","/rest/channel/*"],
...
function deleteUser(requestParams, response, context, ee, next) {
	if( response.statusCode >= 200 && response.statusCode < 300 && response.body.length > 0)  {
		users.filter(u => u.id != context.vars.id);
		fs.writeFileSync('users.data', JSON.stringify(users));
	}
    return next()
}
